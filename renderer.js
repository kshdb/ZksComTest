...
const cmdRepeatFor = $('#cmdRepeatFor');
...
function renderCmdGrid() {
    ...
    cmdGrid.innerHTML = '';
    let repeatingFor =cmdRepeatFor.checked;
    cmdRepeatFor.onchange=async () => {
       const ms = parseInt(cmdRepeatMs.value || '600', 10);
       repeatingFor =cmdRepeatFor.checked;
       await delayedLoop(list, async (cmd, index) => {
        const card = document.createElement('div');
        card.className = 'cmd-card';
        card.dataset.cmdId = cmd.id;
          await sendCommand(cmd, card)
        }, ms);
    }
    list.forEach(cmd => {
        const card = document.createElement('div');
        ...
        btn.onclick = () => {
            if (repeatingFor==false){
            sendCommand(cmd, card);
            }
        }
...
// 重复循环执行
async function delayedLoop(array, callback, delay = 1000) {
  while (cmdRepeatFor.checked) {
    for (const [index, item] of array.entries()) {
        await callback(item, index, array);
        await new Promise(resolve => setTimeout(resolve, delay)); 
        if (cmdRepeatFor.checked==false){
            break
        }
    }
    // console.log('循环完成，重新开始');
  }
}
